...
theory_block = dbc.Collapse(id='theory-block', is_open=False, children=[
    ...
    dbc.Row(children=[
        dbc.Col(md=3, children=[
            html.A(children=[
                html.Img(src='assets/ColvilleRiver.png', title='View on Google Earth', style=image_style),
        ...
        dbc.Col(md=3, children=[
            html.A(children=[
                html.Img(src='assets/SomalianTigerBush.png', title='View on Google Earth', style=image_style),
        ...
        dbc.Col(md=3, children=[
            html.A(children=[
                html.Img(src='assets/PandamatengaZimbabwe.png', title='View on Google Earth', style=image_style),
        ...
        dbc.Col(md=3, children=[
            html.A(children=[
                html.Img(src='assets/RidgeSlough.png', title='View on Google Earth', style=image_style),
...
expert_mode_controls = dbc.Collapse(id='expert-mode-block', is_open=True, children=[
    dbc.Row(children=[
        dbc.Col(md=2, children=[
            dbc.Label('Invert kernel', id='invert-label',
                      html_for='invert-switch'),
            daq.BooleanSwitch(id='invert-switch', on=False)
        ]),
        dbc.Col(md=2, children=[
            dbc.Label('Density correction',
        ...
        dbc.Col(md=2, children=[
            ...
            dcc.Slider(id='interval-length', min=100, max=2000, step=50, value=1000, marks=None,
                       tooltip={'placement': 'bottom', 'always_visible': True}),
        ]),
        dbc.Col(md=2, children=[
...
def make_distance_matrix(target_distance, elongation_parameter, is_continuous_kernel):
    ...
    if not is_continuous_kernel and (np.sum(under_cutoff) > 128):
        ...
        distances = make_distance_matrix(new_target_distance, elongation_parameter, is_continuous_kernel)
...
def build_kernels(model_preset, kernel_function, invert_kernel, scaling_parameter, elongation_parameter,
    ...
    if kernel_function == 'power':
        ...
        if scaling_parameter < 1:
            scaling_parameter = 1
        target_distance = (1 - target_cumulative_probability) ** (-1 / scaling_parameter)
    ...
    distance = make_distance_matrix(target_distance, elongation_parameter, is_continuous_kernel=False)
    ...
    c_distance = make_distance_matrix(target_distance, elongation_parameter, is_continuous_kernel=True)
...
def run_model(density_correction, change_per_iteration, landscape, kernel, max_facilitation, target_density, ):
    ...
    landscape_trace = go.Heatmap(z=landscape, zmin=0, zmax=1,
                                 colorscale=[(0, 'white'), (0.5, 'white'), (0.5, 'black'), (1, 'black')], colorbar=dict(
            title="Cell Type",
            tickvals=[0.25, .75],
...
def max_reached(n_intervals):
    if n_intervals and (n_intervals >= max_dash_interval):
        return True
    return False
