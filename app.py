...
This script produces a Dash app that generates synthetic patterned landscapes using a kernel based stochastic model.
...
app = Dash(update_title=None, title='Patterned Landscapes', external_stylesheets=[dbc.themes.MORPH], server=server)
        ...
        dbc.Col(md=2, children=[
            dbc.Collapse([dbc.Label('Wavelength', id='wavelength-parameter-label', html_for='wavelength-parameter'),
                          dcc.Slider(id='wavelength-parameter', min=1, max=10, step=0.1, value=5,
                                     tooltip={'placement': 'bottom', 'always_visible': True})],
                         id='wavelength-collapse', is_open=True)]),
    ...
    dbc.Collapse([dbc.Row(children=[
        ...
        dbc.Col(md=2,
                children=[
                    ...
                    dcc.Slider(id='density-correction', min=0, max=5, step=0.1, value=1,
                               tooltip={'placement': 'bottom', 'always_visible': True}), ]),
        dbc.Col(md=2, children=[
...
def run_model(density_correction, change_per_iteration, landscape, kernel, max_facilitation, target_density, ):
    ...
    landscape_trace = go.Heatmap(z=landscape, zmin=0, zmax=1, colorscale=[(0, 'white'), (0.5, 'white'), (0.5, 'black'), (1, 'black')], colorbar=dict(
                                    title="Cell Type",
                                    tickvals=[0.25, .75],
                                    ticktext=["Empty", "Vegetated"],
                                    lenmode="pixels", len=100,
                                    outlinewidth=1,))
    landscape_layout = go.Layout({'plot_bgcolor': 'rgba(0,0,0,0)',
                                 'paper_bgcolor': 'rgba(0,0,0,0)'},
...
def interval_options(n_clicks, length, ):
    if not n_clicks:
        return length, True, {'background-color': '#40d933', 'color': 'white', 'line-height': 0}, 'Start'
    elif (n_clicks % 2) == 0:
        return length, True, {'background-color': '#40d933', 'color': 'white', 'line-height': 0}, 'Start'
    return length, False, {'background-color': '#FF4500', 'color': 'white', 'line-height': 0}, 'Stop'
